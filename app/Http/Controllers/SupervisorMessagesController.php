...
class SupervisorMessagesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Open the most recently active client immediately, like a normal messaging
        // inbox. An explicit client query parameter always wins.
        $clientUserId = (int) $request->integer('client')
            ?: (int) ($clients->first()->client_user_id ?? 0);
        $messages = collect();

        if ($clientUserId && $selectedAgentId) {
            // Get all managers who might be involved in this client's conversation.
            $managers = DB::table('users')
                ->where('type', 'manager')
                ->pluck('id')
                ->toArray();

            $supportUserIds = collect([$selectedAgentId, $supId])
                ->merge($managers)
                ->filter()
                ->unique()
                ->values();

            // A chat must always have the selected client at one end. Previously,
            // supervisor/DEO and manager/DEO messages were included here as well,
            // making unrelated history appear in every client's chat.
            $messages = Message::query()
                ->leftJoin('users as s', 's.id', '=', 'messages.sender_id')
                ->select(['messages.*', DB::raw('s.type as sender_type')])
                ->where(function ($query) use ($clientUserId, $supportUserIds) {
                    $query->where('messages.sender_id', $clientUserId)
                        ->whereIn('messages.receiver_id', $supportUserIds);
                })
                ->orWhere(function ($query) use ($clientUserId, $supportUserIds) {
                    $query->whereIn('messages.sender_id', $supportUserIds)
                        ->where('messages.receiver_id', $clientUserId);
                })
