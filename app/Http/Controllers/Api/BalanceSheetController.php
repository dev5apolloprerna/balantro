...
class BalanceSheetController extends Controller
{
	...
	/*public function index_new(Request $request)
    {
        try {
            ...
            // --- Build response exactly like before: data[cr], data[dr], totals ---
            $data = [
                'cr' => [],
                'dr' => [],
            ];
            ...
            foreach ($details as $row) {
                $entry = [
                    ...
                    "decMainAmount"   => $this->fmt((float) $row->decMainAmount),
            ...
            // Pull totals from 2nd result set; if not present, compute as fallback
            if ($totals && (isset($totals->totalCr) || isset($totals->totalDr))) {
                $data['totalCr'] = $this->fmt((float) ($totals->totalCr ?? 0));
                $data['totalDr'] = $this->fmt((float) ($totals->totalDr ?? 0));
            } else {
                // Fallback: compute totals from details (keeps exact signs)
                $totalCr = 0.0;
                $totalDr = 0.0;
                foreach ($details as $row) {
                    if (strtoupper($row->Side ?? '') === 'CR') {
                        $totalCr += (float) $row->decMainAmount;
                    } else {
                        $totalDr += (float) $row->decMainAmount;
                    }
                }
                // Your PHP used: $data['totalDr'] = (-1) * round($totalDr, 2);
                $data['totalCr'] = $this->fmt($totalCr);
                $data['totalDr'] = -1 * $this->fmt($totalDr);
            }
