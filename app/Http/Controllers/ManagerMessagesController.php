...
class ManagerMessagesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Get all actors involved in this client's conversation
        $actors = $this->getConversationActors($toUserId, $managerId);
        ...
        try {
            ...
            if (!empty($files)) {
                $this->handleFileUploads($messageId, $files);
            }

            // DB::commit();

            // 3) Send notifications to all relevant users (after successful DB commit)
            $this->sendNotifications($manager, $toUserId, $validated['description'] ?? '', $actors, $hasFiles);
        } catch (\Throwable $e) {
            // DB::rollBack();
            report($e);
    ...
    private function getConversationActors(int $clientId, int $managerId): array
    {
        // DEOs linked to this client
        $deoIds = DB::table('clients_data_entry_operators')
            ->where('client_id', $clientId)
    ...
    private function sendNotifications($manager, int $clientId, string $messageText, array $actors, bool $hasFiles): void
    {
        try {
            $client = User::find($clientId);
            $messagePreview = $this->getMessagePreview($messageText, $hasFiles);

            // Remove manager from recipients (shouldn't notify themselves)
            $recipientIds = array_filter($actors, fn($id) => $id !== $manager->id);

            if (empty($recipientIds)) {
                return;
            }

            $title = "New message from {$manager->name}";
