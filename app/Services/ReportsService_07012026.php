    ...
    public function balanceSheet(?string $partyguid = null, int $partyId, ?string $from = null, ?string $to = null): array
	{
		try {
			...
			try {
				...
				Log::warning('Failed to fetch closing stock', ['error' => $e->getMessage()]);
			}
			...
			foreach ($rows as $r) {
				...
				if (stripos($grp, 'asset') !== false) {
					$assets += $amt;
				} elseif (stripos($grp, 'liabil') !== false) {
					$liab += $amt;
				} elseif (stripos($grp, 'equity') !== false || stripos($grp, 'capital') !== false) {
					$equity += $amt;
				}
			}

			// Add closing stock to assets
			$assets += $closingStockAmount;

			$totals = [
				'assets'      => $this->fmt($assets),
				'liabilities' => $this->fmt($liab),
			...
			return [
				'success' => false,
				'message' => 'Failed to build Balance Sheet: ' . $e->getMessage(),
				'error'   => $e->getMessage(),
