...
<script>
        ...
        $.get(url, function(res) {
            ...
            setTimeout(() => {
                $('#itemsBody tr').each(function(i){
                    ...
                    let ledgerId = item.ledger_id ? String(item.ledger_id) : '';
                    if (!ledgerId && item.ledger_name) {
                        let matchedLedger = ledgers.find(l => clean(l.name) === clean(item.ledger_name));
                        ledgerId = matchedLedger ? String(matchedLedger.id) : '';
                    }
                    select.val(ledgerId).trigger('change.select2');
                });
            }, 100);
    ...
    function rowHtml(item = {}, index = '') {
        ...
        let selectedLedgerId = item.ledger_id ? String(item.ledger_id) : '';

        if (!selectedLedgerId && item.ledger_name) {
            let matchedLedger = ledgers.find(l => clean(l.name) === clean(item.ledger_name));
            selectedLedgerId = matchedLedger ? String(matchedLedger.id) : '';
        }
        ledgers.forEach(l => {
            let ledgerId = String(l.id);
            let selected = ledgerId === selectedLedgerId ? 'selected' : '';
            options += `<option value="${ledgerId}" ${selected}>${l.name}</option>`;
        });
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.super_admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp\htdocs\balantro\resources\views/admin/bulkupload/journal/preview.blade.php ENDPATH**/ ?>