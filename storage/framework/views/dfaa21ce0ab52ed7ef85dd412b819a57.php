...
<script>
    ...
    function applyItemGstMapping(itemId = '', force = false) {
        ...
        let normalizedItem = String(itemId || '').trim().toLowerCase();
        if (!normalizedItem) return;

        $('#customSlotsBody tr').each(function() {
            let slotKey = String($(this).data('sales-ledger-id') || $(this).find('.slot_sales_ledger_id').val() || '').trim().toLowerCase();
            if (!slotKey || slotKey !== normalizedItem) {
                return;
            }
