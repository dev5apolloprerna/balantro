...
<div class="h-full bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-2xl overflow-hidden flex flex-col">
    ...
    <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
        <div class="flex items-center gap-3">
            <?php $initials = strtoupper(substr($clientName ?? 'U', 0, 2)); ?>
        ...
        <div class="flex items-center gap-1 text-gray-600 dark:text-gray-300">
            <button type="button" class="p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700" title="Search in chat">
        ...
        <?php $__currentLoopData = $messages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $msg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <div class="mt-1 flex <?php echo e($isMe ? 'justify-end' : 'justify-start'); ?>">
                <div class="mr-2 mt-0.5 shrink-0">
                    <div class="w-7 h-7 rounded-full grid place-items-center text-[11px] font-semibold <?php echo e($isMe ? 'bg-indigo-600 text-white' : 'bg-gray-300 dark:bg-gray-700 text-gray-700 dark:text-gray-200'); ?>">
                        <?php echo e($roleLetter($msg)); ?>

                    </div>
                </div>

                <div class="max-w-[78%] md:max-w-[66%] px-3 py-2 rounded-2xl leading-snug <?php echo e($isMe ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-gray-300 dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-bl-none'); ?>">
                                ...
                                <?php if($url && $url !== '#'): ?>
                                    ...
                                    <?php else: ?>
                                        <a href="<?php echo e($url); ?>" target="_blank" rel="noopener"
                                            ...
                                            <span class="text-xs truncate max-w-[140px] text-gray-700 dark:text-gray-300"><?php echo e($name); ?></span>
                                            <?php if($sizeTxt): ?>
    ...
    <?php
        $formId = 'supervisorChatForm';
        $attachId = 'supervisorAttach';
        $sendButtonId = 'supervisorSendButton';
    ?>
    ...
    <form id="<?php echo e($formId); ?>" action="<?php echo e(route('supervisor.messages.store')); ?>" method="POST"
        ...
        <div class="px-3 pb-3 pt-1 flex items-end gap-2">
            ...
            <textarea name="description" rows="1" placeholder="Type a message…"
                class="min-h-[44px] max-h-40 flex-1 resize-y rounded-xl px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600 focus:outline-none focus:border-indigo-500"></textarea>

            <button type="submit" id="<?php echo e($sendButtonId); ?>"
                class="shrink-0 px-4 h-11 rounded-xl bg-indigo-600 text-white font-medium cursor-not-allowed" disabled>
                Send
            </button>
        </div>
    </form>
...
</script><?php /**PATH D:\xampp\htdocs\balantro\resources\views\supervisors\messages\chat_content.blade.php ENDPATH**/ ?>