                ...
                <table id="bankTable" class="min-w-[1100px] text-sm text-gray-700 dark:text-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800 tabular-nums">
                        ...
                        <tr data-cheque="<?php echo e($row->cheque_no); ?>"
                            ...
                            data-cost="<?php echo e($row->cost_center); ?>"
                            data-pending-issues='<?php echo json_encode($row->pending_issues ?? [], 15, 512) ?>'  class="group border-b border-gray-200 dark:border-neutral-700 transition-all duration-300 hover:bg-[#22d3ee]/80 dark:hover:bg-[#22d3ee]/80 hover:shadow-[0_0_20px_rgba(34,211,238,0.8)] [&>*]:group-hover:text-black [&_*]:group-hover:text-black <?php echo e($row->is_suspense == 1 ? 'opacity-50 pointer-events-none' : ''); ?>">
                            <td class="px-3 py-2">
                                <input type="checkbox"
                                    name="selected[]"
    ...
    $('#bulkLedger').change(function() {
        ...
        $('#bankTable tbody tr:visible').each(function() {
            ...
            if (checkbox.is(':checked')) {
                ...
                let type = row.find('select[name^="type"]').val().toLowerCase();
                let ledgerDropdown = row.find('select[name^="ledger"]');
                ledgerDropdown.html(getLedgerOptions(type, ledger));
                ledgerDropdown.val(ledger).data('selected', ledger).trigger('change.select2');
            }
        });
    });
