...
<div data-controller="confirm-delete"
    ...
    <div class="container mx-auto">
        <div class="flex justify-between items-center mb-3">
            <h6 class="font-semibold mb-0 dark:text-white"><?php echo e(__("Purchase")); ?>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800 tabular-nums">
                                ...
                                <tr class="group transition-all duration-300 hover:bg-[#22d3ee]/80 dark:hover:bg-[#22d3ee]/80 hover:shadow-[0_0_20px_rgba(34,211,238,0.8)] [&>*]:group-hover:text-black [&_*]:group-hover:text-black">
                                    ...
                                    <td class="px-4 py-3 text-right flex justify-end gap-4">
                                        ...
                                        <form method="POST" action="<?php echo e(route('purchase.rematch', $upload->id)); ?>" class="inline">
                                            <?php echo csrf_field(); ?>
                                            <button
                                                type="submit"
    ...
    <script>
        $(document).ready(function() {
            $('#sampleDownload').change(function() {
                let type = $(this).val();
                if (type == 'with-item') {
                    // window.location.href = window.downloadFile("/samples/Purchase-with-item-sample-file.xlsx");
                    window.downloadFile("/samples/Purchase-with-item-sample-file.xlsx");
                }
                if (type == 'without-item') {
                    // window.location.href = window.downloadFile("/samples/Purchase-without-item-sample-file.xlsx");
                    window.downloadFile("/samples/Purchase-without-item-sample-file.xlsx");
                }
                // reset dropdown
                $(this).val('');
            });
    ...
    <script>
        ...
        function addPurchaseRow(data = {}) {
            ...
            <tr>
                ...
                <td><select class="gst border p-1">${buildGstRateOptions(data.gst || 0)}</select></td>
        ...
        function savePurchase() {
            if (!validateBulkInvoiceFrontend({ headerLedger: '#noitem_purcashe_ledger', ledgerName: 'Purchase Ledger' })) return;
