<?php $sel = $selectedClient ?? ($selected_user ?? ($selected_client ?? ($selected ?? null))); ?>
<form action="<?php echo e(route('supervisor.messages.store')); ?>" method="POST" class="flex flex-col gap-2" data-controller="message"
    data-action="submit->message#submitForm input->message#toggleSubmitButton">
    <?php echo csrf_field(); ?>
    <input type="hidden" name="receiver_id" value="<?php echo e($sel?->id); ?>">
    <input type="hidden" name="is_first_message" value="<?php echo e(!$messages ? 'true' : 'false'); ?>">
...
<?php /**PATH D:\xampp\htdocs\balantro\resources\views\supervisors\messages\message-form.blade.php ENDPATH**/ ?>