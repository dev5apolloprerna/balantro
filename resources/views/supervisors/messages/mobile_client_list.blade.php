<div class="flex flex-col h-full bg-white dark:bg-neutral-800" data-controller="client-selection">
    <input type="hidden" name="selection_mode" value="{{ $selection_mode ?? false }}"
    ...
    <div class="flex items-center justify-between gap-2 px-4 py-3 border-b border-neutral-200 dark:border-neutral-700">
        <div class="flex items-center gap-3">
            <img src="{{ asset('light-logo.svg') }}" alt="light-logo" class="light-logo block dark:hidden">
            <img src="{{ asset('dark-logo.svg') }}" alt="dark-logo" class="dark-logo hidden dark:block">
        </div>
    ...
    <div class="chat-search w-full relative px-4 py-3 border-b border-neutral-200 dark:border-neutral-700">
        ...
        <form action="{{ route('supervisor.messages.index') }}" method="GET" data-turbo-frame="mobile_chat"
        ...
        @if ($clients->count() > 0)
            @foreach ($clients as $client)
                <div class="flex items-center gap-3 px-4 py-3 hover:bg-neutral-100 dark:hover:bg-neutral-700 cursor-pointer"
                    ...
                    data-client-selection-client-id-value="{{ $client->client_user_id }}">
                    <div class="flex items-center gap-2 w-full" data-controller="update-recepient"
                        data-update-recepient-recipient-id-value="{{ $client->client_user_id }}">
                        <div class="flex items-center justify-center w-5 h-5 rounded border border-neutral-300 dark:border-neutral-500 bg-white dark:bg-neutral-700 mr-2 shrink-0 transition-all duration-200 relative {{ $selection_mode ? '' : 'hidden' }}"
                            data-client-selection-target="checkbox">
                            <input type="checkbox" name="selected_clients[]" value="{{ $client->client_user_id }}"
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="text-sm font-medium line-clamp-1">{{ $client->client_name }}</div>
    ...
    <div class="sticky bottom-0 z-10 p-4 bg-white dark:bg-neutral-800 border-t border-neutral-200 dark:border-neutral-700 {{ $selection_mode ? '' : 'hidden' }}"
            ...
            <div id="selected-clients-fields" data-client-selection-target="selectedClientsFields">
                @if ($selection_mode)
                    @foreach ($selectedClients as $clientId)
                        <input type="hidden" name="selected_clients[]" value="{{ $clientId }}">
                    @endforeach
                @else
                    <input type="hidden" name="receiver_id" value="{{ $selectedclient->client_user_id ?? '' }}">
                @endif
            </div>
