{{-- resources/views/supervisors/messages/chat_content.blade.php --}}
<div class="h-full bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-2xl overflow-hidden flex flex-col">
    ...
    <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
        <div class="flex items-center gap-3">
            @php $initials = strtoupper(substr($clientName ?? 'U', 0, 2)); @endphp
        ...
        @foreach ($messages as $msg)
            ...
            <div class="mt-1 flex {{ $isMe ? 'justify-end' : 'justify-start' }}">
                <div class="mr-2 mt-0.5 shrink-0">
                    <div class="w-7 h-7 rounded-full grid place-items-center text-[11px] font-semibold {{ $isMe ? 'bg-indigo-600 text-white' : 'bg-gray-300 dark:bg-gray-700 text-gray-700 dark:text-gray-200' }}">
                        {{ $roleLetter($msg) }}
                    </div>
                </div>

                <div class="max-w-[78%] md:max-w-[66%] px-3 py-2 rounded-2xl leading-snug {{ $isMe ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-gray-300 dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-bl-none' }}">
                                ...
                                @if ($url && $url !== '#')
                                    ...
                                    @else
                                        <a href="{{ $url }}" target="_blank" rel="noopener"
                                            ...
                                            <span class="text-xs truncate max-w-[140px] text-gray-700 dark:text-gray-300">{{ $name }}</span>
                                            @if ($sizeTxt)
