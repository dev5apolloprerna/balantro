<div class="container mx-auto"
     ...
     data-document-url-value="{{ route('supervisors.documents.index') }}"
     data-document-current-page-value="{{ request('page', 1) }}"
     data-document-bulk-update-url-value="{{ route('supervisors.documents.bulk_update') }}"
     data-document-bulk-update-current-page-value="{{ request('page', 1) }}">
  <div class="flex justify-between items-center mb-6">
    <h6 class="font-semibold mb-0 dark:text-white">{{ __('supervisors.documents.index.title') }}</h6>
  </div>
            ...
            <table class="table bordered-table mb-0">
              <thead>
                <tr>
                  ...
                  @endcan
                  <th scope="col" class="text-center">{{ __('supervisors.documents.index.table_headers.document_name') }}</th>
                  <th scope="col" class="text-center">{{ __("admin.documents.table.upload_date") }}</th>
                  <th scope="col" class="text-center">{{ __('supervisors.documents.index.table_headers.client_name') }}</th>
                  <th scope="col" class="text-center">{{ __('supervisors.documents.index.table_headers.status') }}</th>
                  <th scope="col" class="!text-center">{{ __('supervisors.documents.index.table_headers.actions') }}</th>
                </tr>
              </thead>
              <tbody id="{{ auth()->user()->role }}_doc_listing">
                @if($documents->count() > 0)
                  @foreach($documents as $document)
                    @include('supervisors.documents.document', ['document' => $document])
                  @endforeach
                @else
                  <tr>
                    <td colspan="6" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                      <div class="flex flex-col items-center justify-center">
                        <iconify-icon icon="heroicons-outline:document-magnifying-glass" class="text-4xl text-neutral-400 mb-3" title="{{ __('supervisors.documents.index.no_documents_found.icon_title') }}"></iconify-icon>
                        <p class="text-lg font-medium mb-1">{{ __('supervisors.documents.index.no_documents_found.message') }}</p>
                        <p class="text-sm">
                          {{ __("admin.documents.table.no_documents_description") }}
                        </p>
                      </div>
                    </td>
                  </tr>
