<div class="flex flex-col h-full bg-white dark:bg-neutral-800" data-controller="client-selection">
    <input type="hidden" name="selection_mode" value="{{ $selectionMode ?? false }}"
    ...
    <div class="chat-search w-full relative px-4 py-3 border-b border-neutral-200 dark:border-neutral-700">
        ...
        <form action="{{ route('managers.messages.index') }}" method="GET" data-turbo-frame="mobile_chat"
            ...
            <input type="text" name="search" value="{{ request('search') }}"
                ...
                autocomplete="off" placeholder="{{ __('chat.search.placeholder') }}">
        </form>
    </div>
    ...
    <div id="selected-clients-container"
        ...
        <label class="flex items-center gap-2 cursor-pointer shrink-0">
            ...
            <span class="text-sm">{{ __('chat.client_list.select_all') }}</span>
        </label>
    </div>
        ...
        @if (count($clients) > 0)
            @foreach ($clients as $client)
                <div class="flex items-center gap-3 px-4 py-3 hover:bg-neutral-100 dark:hover:bg-neutral-700 cursor-pointer"
                    ...
                    data-client-selection-client-id-value="{{ $client->id }}">
                    <div class="flex items-center gap-2 w-full" data-controller="update-recepient"
                        data-update-recepient-recipient-id-value="{{ $client->id }}">
                        <div class="flex items-center justify-center w-5 h-5 rounded border border-neutral-300 dark:border-neutral-500 bg-white dark:bg-neutral-700 mr-2 shrink-0 transition-all duration-200 relative {{ !$selectionMode ? 'hidden' : '' }}"
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="text-sm font-medium line-clamp-1">{{ $client->name }}</div>
        ...
        @else
            <div class="flex flex-col items-center justify-center h-full text-center py-10 px-6">
                ...
                <div class="text-lg font-medium text-neutral-800 dark:text-white mb-1">
                    {{ __('chat.client_list.no_clients') }}
                </div>
                <p class="text-neutral-600 dark:text-neutral-400 text-sm">
                    @if (request('search'))
                        {{ __('chat.client_list.no_results', ['search' => request('search')]) }}
                    @else
                        {{ __('chat.client_list.no_clients_available') }}
                    @endif
                </p>
            </div>
        @endif
    ...
    <div class="sticky bottom-0 z-10 p-4 bg-white dark:bg-neutral-800 border-t border-neutral-200 dark:border-neutral-700 {{ !$selectionMode ? 'hidden' : '' }}"
        data-client-selection-target="footer">
        <form action="{{ route('managers.messages.store') }}" method="POST" class="flex flex-col gap-2"
            ...
            <div id="selected-clients-fields" data-client-selection-target="selectedClientsFields">
                @if ($selectionMode)
                    @foreach ((array) $selectedClients as $clientId)
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="button"
                    ...
                    title="{{ __('chat.messages.file_input.attach') }}">
                ...
                <div class="flex-1 relative">
                    <textarea name="description" rows="1"
                        ...
                        placeholder="{{ __('chat.messages.input.placeholder') }}" autocomplete="off"
