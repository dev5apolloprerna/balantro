...
<div class="h-full flex flex-col bg-white dark:bg-black border border-gray-300 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm dark:shadow-none">
    ...
    <form id="{{ $formId }}" action="{{ route('manager.messages.store') }}" method="POST"
        enctype="multipart/form-data" data-loader="false" class="flex-shrink-0 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
        @csrf
...
<script>
    ...
    (function() {
        ...
        function appendOutgoingMessage(text) {
            ...
            row.innerHTML = `<div class="mr-2 mt-0.5 shrink-0"><div class="w-7 h-7 rounded-full grid place-items-center text-[11px] font-semibold bg-indigo-600 text-white">M</div></div><div class="max-w-[78%] md:max-w-[66%] px-3 py-2 rounded-2xl leading-snug bg-indigo-600 text-white rounded-br-none"><div class="flex items-end justify-between gap-2"><p class="whitespace-pre-line break-words flex-1 leading-snug"></p><span class="text-[10px] opacity-80 shrink-0 ml-2 whitespace-nowrap self-end"></span></div></div>`;
            row.querySelector('p').textContent = text;
            row.querySelector('span').textContent = new Date().toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit',
                hour12: false
            });
            mobileScroll?.appendChild(row);
            if (mobileScroll) mobileScroll.scrollTop = mobileScroll.scrollHeight;
            return row;
        }
        // Clear file input
        // input.value = '';
