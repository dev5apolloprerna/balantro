{{-- managers/messages/chat_content.blade.php --}}
<style>
...
<div class="h-full bg-black border border-gray-800 rounded-2xl overflow-hidden flex flex-col">
    ...
    <div class="flex items-center justify-between px-4 py-3 border-b border-gray-800 bg-black">
        <div class="flex items-center gap-3">
            @php $initials = strtoupper(substr($selected_client->name ?? 'U', 0, 1)); @endphp
            <div class="w-9 h-9 rounded-full bg-indigo-600 text-white grid place-items-center font-semibold">
                {{ $initials }}
            </div>
            <div class="text-white font-medium">{{ $selected_client->name ?? 'Client' }}</div>
        </div>
        ...
        @foreach ($messages as $msg)
            ...
            <div class="mt-1 flex {{ $isMe ? 'justify-end' : 'justify-start' }}">
                <div class="mr-2 mt-0.5 shrink-0">
                    <div class="w-7 h-7 rounded-full grid place-items-center text-[11px] font-semibold {{ $isMe ? 'bg-indigo-600 text-white' : 'bg-gray-700 text-gray-200' }}">
                        {{ $roleLetter($msg) }}
                    </div>
                </div>

                <div class="max-w-[78%] md:max-w-[66%] px-3 py-2 rounded-2xl leading-snug {{ $isMe ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-gray-800 text-gray-100 rounded-bl-none' }}">
