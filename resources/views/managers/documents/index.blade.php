<div class="container mx-auto" data-controller="document" data-document-url-value="{{ route('managers.documents.index') }}">
  <div class="flex justify-between items-center mb-6">
    <h6 class="font-semibold mb-0 dark:text-white">{{ __('managers.documents.index.title') }}</h6>
  </div>
      ...
      <div class="card !border-0 rounded-lg overflow-hidden">
        <div class="card-header border-b border-neutral-200 dark:border-neutral-600 bg-white dark:bg-neutral-700 p-4 sm:p-6">
          <div class="flex flex-col gap-3">
            <div class="flex flex-col sm:flex-row gap-3 w-full items-start">
              <form action="{{ route('managers.documents.index') }}" method="GET" class="w-full" id="documents-filter-form">
                <div class="flex flex-col md:flex-row md:flex-wrap gap-3 w-full">
                    ...
                    <select name="status" class="custom-select-arrow bg-white dark:bg-neutral-800 text-neutral-800 dark:text-neutral-100 h-10 px-4 !rounded-lg focus:outline-none border border-neutral-300 dark:border-neutral-600 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 text-[16px] w-full cursor-pointer">
                      ...
                      @foreach($statuses as $status)
                        <option value="{{ $status }}" {{ request('status') == $status ? 'selected' : '' }}>
                          {{ __('admin.documents.statuses.' . $status) }}
                        </option>
                      @endforeach
                    </select>
                  </div>
                  ...
                  @if (request('client_id') || request('status') || request('start_date') || request('end_date'))
                    <a href="{{ route('managers.documents.index') }}"
        ...
        <div class="card-body">
          <div class="table-responsive">
            <table class="table bordered-table mb-0">
              <thead>
                <tr>
                  <th scope="col">{{ __('managers.documents.index.table_headers.document_name') }}</th>
                  <th scope="col">{{ __('admin.documents.table.upload_date') }}</th>
                  <th scope="col">{{ __('managers.documents.index.table_headers.client_name') }}</th>
                  <th scope="col">{{ __('managers.documents.index.table_headers.status') }}</th>
                  <th scope="col" class="!text-center">{{ __('managers.documents.index.table_headers.actions') }}</th>
                </tr>
              </thead>
              <tbody>
                @if ($documents->count())
                  @foreach($documents as $document)
                    @include('managers.documents._document', ['document' => $document])
                  @endforeach
                @else
                  <tr>
                    <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                      <div class="flex flex-col items-center justify-center">
                        <iconify-icon icon="heroicons-outline:document-magnifying-glass" class="text-4xl text-neutral-400 mb-3" title="{{ __('managers.documents.index.no_documents_found.icon_title') }}"></iconify-icon>
                        <p class="text-lg font-medium mb-1">{{ __('managers.documents.index.no_documents_found.message') }}</p>
                      </div>
                    </td>
                  </tr>
                @endif
