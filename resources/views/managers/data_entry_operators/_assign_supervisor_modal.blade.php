<div class="relative p-4 w-full max-w-2xl max-h-full">
  <div class="relative bg-white dark:bg-dark-2 rounded-2xl shadow-lg">
    ...
    <div class="flex items-center justify-between p-4 md:p-5 border-b border-gray-200 dark:border-gray-600 rounded-t">
      <h6 class="font-semibold text-gray-900 dark:text-white">{{ __('managers.data_entry_operators.assign_modal.title') }}</h6>
      <button type="button"
        ...
        <span class="sr-only">{{ __('managers.data_entry_operators.assign_modal.close_btn') }}</span>
      </button>
    </div>
    <!-- Modal Body -->
    <div class="p-4 md:p-5 space-y-4">
      <div class="mb-3">
        <label class="block text-sm font-semibold text-neutral-600 dark:text-neutral-200 mb-2">{{ __('managers.data_entry_operators.supervisor_label') }}</label>
        <select name="supervisor"
          ...
          @foreach($managerSupervisors as $supervisor)
            <option value="{{ $supervisor->id }}" {{ in_array($supervisor->id, $selectedSupervisorIds) ? 'selected' : '' }}>
              {{ $supervisor->name }}
            </option>
          @endforeach
        </select>
    ...
    <div class="flex justify-end gap-4 p-4 md:p-5 border-t border-gray-200 dark:border-gray-600 rounded-b">
      <button type="button"
              ...
              data-action="click->manager--data-entry-operator--allocate-supervisor#hideModal">
        {{ __('managers.data_entry_operators.assign_modal.cancel_btn') }}
      </button>
      <button type="button"
        ...
        {{ __('managers.data_entry_operators.assign_modal.assign_btn') }}
      </button>
    </div>
  </div>
</div>