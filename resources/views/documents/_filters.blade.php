      ...
      <form method="GET" action="{{ route('documents.index') }}" class="w-full" data-turbo-frame="client-documents-list">
        <div class="flex flex-col md:flex-row md:flex-wrap gap-3 w-full items-start">
          ...
          @if(request()->status || request()->start_date || request()->end_date)
            <a href="{{ route('documents.index') }}" class="w-full sm:w-auto self-start flex-none items-center btn border border-danger-600 bg-hover-danger-200 !text-danger-500 h-10 px-4 rounded-lg text-[16px] whitespace-nowrap justify-center cursor-pointer">
              {{ __('search.reset') }}
            </a>
          @endif
        </div>
