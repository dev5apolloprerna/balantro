<div class="grid">
  <div class="card h-full p-0 border-0 overflow-hidden">
    <div class="card-body p-6">
      <form method="POST" action="{{ route('documents.store') }}" id="document-upload-form" enctype="multipart/form-data" class="flex flex-col gap-4" data-turbo="true" data-action="turbo:submit-start->document-upload#showLoader turbo:submit-end->document-upload#hideLoader">
        @csrf
        <input type="hidden" name="document[user_id]" value="{{ auth()->id() }}">
        ...
        <div class="border-2 border-dashed border-neutral-400 rounded-xl p-6 text-center 
          ...
          <p class="mb-2">
            ...
            {{ __('client.documents.form.drag_and_drop_instruction') }}
          </p>
        </div>
        ...
        <div class="flex justify-end">
          <button type="submit" 
            ...
            {{ __('client.documents.form.upload_button') }}
          </button>
        </div>
