  ...
  <form method="POST" action="{{ route('documents.update', $document) }}" enctype="multipart/form-data" 
    ...
    <div class="file-info" data-document-upload-target="uploadedList">
        ...
        <div class="flex items-center gap-2 flex-1 min-w-0">
            ...
            <span class="truncate block whitespace-nowrap overflow-hidden text-ellipsis" title="{{ $document->file->filename }}">{{ $document->file->filename }}</span>
        </div>
        <span class="text-neutral-500 text-sm ml-2">{{ number_to_human_size($document->file->byte_size) }}</span>
        </li>
    </div>
    <!-- Submit Button -->
