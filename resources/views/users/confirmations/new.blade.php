  ...
  <div class="font-sans w-full lg:w-1/2 py-8 px-4 sm:px-6 flex flex-col justify-center">
    <div class="w-full max-w-md mx-auto lg:max-w-[464px] px-4 sm:px-6">
      <div class="text-center lg:text-left">
        ...
        <h6 class="mb-3">{{ __('Resend Confirmation Instructions') }}</h6>
        <p class="mb-6 text-secondary-light text-base sm:text-lg">{{ __('Enter your email to resend confirmation instructions') }}</p>
        @if (session('alert'))
        ...
        <div class="relative mb-3 sm:mb-4">
          <div class="icon-field relative">
            ...
            <input id="email" type="email" name="email" 
                   value="{{ old('email', $user->pending_reconfirmation ? $user->unconfirmed_email : $user->email) }}"
          ...
          @error('email')
            <p class="form-error text-xs sm:text-sm text-red-600 mt-1">{{ __('Email') }} {{ $message }}</p>
          @enderror
        </div>
        
        <button type="submit" class="btn btn-primary justify-center h-11 sm:h-13 w-full text-sm sm:mt-2 cursor-pointer">
          {{ __('Resend Confirmation Instructions') }}
        </button>
