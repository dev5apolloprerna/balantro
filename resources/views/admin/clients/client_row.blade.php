<tr id="client_{{ $client->id }}"
    ...
    data-client-id="{{ $client->id }}" data-client-mobile="{{ $client->profile?->mobile_no }}"
    ...
    <td class="px-2 py-1  text-neutral-700 dark:text-neutral-300">
        ...
        @else
            <span
                ...
                {{ __('admin.clients.table.no_managers') }}
            </span>
        @endif
    </td>
    ...
    <td class="px-2 py-1  text-neutral-700 dark:text-neutral-300">
        ...
        @else
            <span
                class="inline-flex rounded-full bg-info-100 px-3 py-1 text-xs font-semibold text-info-800 dark:bg-info-900/40 dark:text-info-300">
                {{ __('admin.clients.table.no_supervisors') }}
            </span>
        @endif
    </td>
    ...
    <td class="px-2 py-1  text-neutral-700 dark:text-neutral-300">
        ...
        @else
            <span
                class="inline-flex rounded-full bg-info-100 px-3 py-1 text-xs font-semibold text-info-800 dark:bg-info-900/40 dark:text-info-300">
                {{ __('admin.clients.table.no_data_entry_operators') }}
            </span>
        @endif
    </td>

    <td class="px-2 py-1  text-neutral-700 dark:text-neutral-300">
        {{-- {{ $client->groups->pluck('name')->join(', ') ?: '-' }} --}}
    ...
    <td class="px-2 py-1 ">
        @if ($user->role === \App\Models\User::ROLES['super_admin'])
            <form method="POST" action="{{ route('clients.toggleStatus', $client) }}"
                onsubmit="return confirm('Are you sure you want to {{ $client->is_active ? 'deactivate' : 'activate' }} this client?')">
                @csrf
                @method('PATCH')
                <button type="submit"
                 ...
                 {{ $client->is_active
                    ...
                    title="{{ $client->is_active ? 'Mark Inactive' : 'Mark Active' }}">
    ...
    <td class="px-2 py-1 align-middle">
        <div class="flex items-center justify-center gap-2">
            @if ($user->role === \App\Models\User::ROLES['super_admin'])
                @can('clients.assign-groups')
                    <button type="button"
                        ...
                        title="Assign Groups" data-modal-open="assign-groups" data-client-id="{{ $client->id }}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                            <path fill="currentColor"
                ...
                @can('clients.assign-permissions')
                    <button type="button"
            ...
            @can('clients.manager-supervisors')
                <button type="button"
                    ...
                    title="{{ __('admin.clients.table.assign_users_btn') }}" data-modal-open="assign-users"
                    data-client-id="{{ $client->id }}" data-manager-id="{{ $client->managers->first()?->id }}"
                    data-supervisor-id="{{ $client->supervisors->first()?->id }}"
                    data-deo-id="{{ $client->dataEntryOperators->first()?->id }}">
                    <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
            ...
            @if ($user->role === \App\Models\User::ROLES['super_admin'])
                @can('clients.update')
                    <button type="button" data-modal-open="client-form" data-client-id="{{ $client->id }}"
                ...
                {{-- @can('destroy', App\Http\Controllers\Admin\ClientsController::class) --}}
                {{-- <form action="{{ route('clients.destroy', $client) }}" method="POST"
                    ...
                    <button type="submit"
                        ...
                        title="{{ __('admin.clients.table.delete_btn') }}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                            <g fill="none">
                                <path
                ...
                @can('clients.delete')
                    {{-- <button type="button" title="Delete"
                        class="rounded-full bg-rose-100 p-2 text-rose-700 ring-1 ring-inset ring-rose-200 hover:bg-rose-200 dark:bg-rose-900/30 dark:text-rose-300 dark:ring-rose-800"
                        data-open-delete data-id="{{ $client->id }}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                            <g fill="none">
                                <path
                                    d="m12.593 23.258l-.011.002l-.071.035l-.02.004l-.014-.004l-.071-.035q-.016-.005-.024.005l-.004.01l-.017.428l.005.02l.01.013l.104.074l.015.004l.012-.004l.104-.074l.012-.016l.004-.017l-.017-.427q-.004-.016-.017-.018m.265-.113l-.013.002l-.185.093l-.01.01l-.003.011l.018.43l.005.012l.008.007l.201.093q.019.005.029-.008l.004-.014l-.034-.614q-.005-.018-.02-.022m-.715.002a.02.02 0 0 0-.027.006l-.006.014l-.034.614q.001.018.017.024l.015-.002l.201-.093l.01-.008l.004-.011l.017-.43l-.003-.012l-.01-.01z">
            ...
            @endif
            @if ($client->guid != '')
                <a href="{{ route('clients.dashboard', $client->guid ?? '') }}"
                ...
                <a href="{{ route('clients.Gstindex', $client->guid) }}" class="rounded-full bg-cyan-100 p-2 text-cyan-700 ring-1 ring-inset ring-cyan-200 hover:bg-cyan-200 dark:bg-cyan-900/30 dark:text-cyan-300 dark:ring-cyan-800" 
                    title="GST Settings">
                ...
                {{-- <div class="relative group">
                    <button type="button"
                        class="rounded-full bg-teal-100 p-2 text-teal-700 ring-1 ring-inset ring-teal-200 hover:bg-teal-200 dark:bg-teal-900/30 dark:text-teal-300 dark:ring-teal-800"
                    ...
                    <div
                        class="absolute right-0 mt-2 hidden w-40 rounded-md border border-gray-200 bg-white shadow-lg group-hover:block dark:border-gray-700 dark:bg-gray-800">
                        <a href="{{ route('clients.reports.pnl', $client->guid) }}"
                            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700">
                            Profit & Loss
                        </a>
                        <a href="{{ route('clients.reports.balanceSheet', $client->guid) }}"
                            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700">
                            Balance Sheet
                        </a>
                        <a href="{{ route('clients.reports.ledger', $client->guid) }}"
                            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700">
                            Ledger
                        </a>
                    </div>
