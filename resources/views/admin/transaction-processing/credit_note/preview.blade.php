...
<div class="container mx-auto">
    <div class="bg-white dark:bg-neutral-900 rounded-lg shadow border border-gray-200 dark:border-neutral-700">
        ...
        <div class="flex justify-between items-center px-5 py-3 border-b border-neutral-700">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('transaction_processing.processing_credit_note') }}"
                        ...
                        <tr class="group transition-all duration-300 hover:bg-[#22d3ee]/80 dark:hover:bg-[#22d3ee]/80 hover:shadow-[0_0_20px_rgba(34,211,238,0.8)] [&>*]:group-hover:text-black [&_*]:group-hover:text-black">
                            ...
                            <td>
                                ...
                                <select name="ledger[{{$row->id}}]"
                                    ...
                                    data-selected="{{ $row->sales_ledger ?: $row->party_name }}">
                                    <option value="{{ $row->sales_ledger ?: $row->party_name }}" selected>{{ $row->sales_ledger ?: $row->party_name ?: 'Select Ledger' }}</option>
                                </select>
                            </td>
                            <td>
                                {{$row->gst_no}}
...
<script>
    ...
    $(document).ready(function() {
        ...
        window.hydrateDeferredSelectOptions?.('.credit-note-table .js-deferred-options');
    ...
    $('#saveBtn').click(function() {
        ...
        if (missingLedgerRows.length) {
            showToast('Please select ledger for all selected credit note rows before submitting. Missing ledger on row(s): ' + missingLedgerRows.join(', '), 'error');
            return;
        }
        let formData = $('#salesForm').serialize();
        $.ajax({
            url: "{{ route('transaction_processing.credit_note_sumbit') }}",
            type: "POST",
            data: formData,
            success: function(response) {
    ...
    $(document).on('click', '.deleteRow', function() {
        ...
        $.ajax({
            url: "{{ route('cn.delete', ':id') }}".replace(':id', id),
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}"
            },
    ...
    //     // Load data
    //     $.ajax({
    //         url: "{{ route('cn.show', ':id') }}".replace(':id', id),
    //         type: "GET",
    ...
    function buildReadOnlyItemRow(item) {
        ...
        return `<tr>
            ...
            <td style="width:80px;"><input type="text" class="item-hsn" inputmode="numeric" maxlength="8" pattern="\\d{4}|\\d{6}|\\d{8}" title="HSN Code must contain exactly 4, 6, or 8 digits" value="${item.hsn_code || item.hsn || ''}" readonly style="background:#f3f4f6;width:100%;padding:4px 8px;border:1px solid #e5e7eb;border-radius:4px;text-align:center;"></td>
            <td style="width:65px;"><select class="item-gst_rate" style="width:100%;padding:4px 8px;border:1px solid #e5e7eb;border-radius:4px;text-align:right;">${buildGstRateOptions(item.gst_rate || 0)}</select></td>
   ...
   // ═══════ EDIT MODAL ═══════
    ...
    $('#updateRow').click(function () {
        if (!validateBulkInvoiceFrontend({ headerLedger: '#noitem_sales_ledger', ledgerName: 'Sales Ledger', documentName: 'Credit Note', itemSelector: '.item-name', gstSelector: '.item-gst_rate', qtySelector: '.item-qty', unitSelector: '.item-unit', rateSelector: '.item-rate', amountSelector: '.item-amount', hsnSelector: '.item-hsn' })) return;
        let items = [];
    ...
    function recalcItemRow(row) {
        ...
        let total = roundCurrency(amount) + roundCurrency(cgst) + roundCurrency(sgst) + roundCurrency(igst);
        ...
        if ($('#no_item_section').is(':visible')) {
            ...
            $('#noItemBody tr').each(function(index) {
                ...
                if (isIGST) {
                    ...
                    let halfGst = roundCurrency((rowAmount * gstRate) / 200);
                    cgst += halfGst;
                    sgst += halfGst;
                    rateMap[rateKey].cgst += halfGst;
                    rateMap[rateKey].sgst += halfGst;
                }
            });
