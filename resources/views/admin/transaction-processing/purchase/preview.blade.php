    ...
    <div class="bg-white dark:bg-neutral-900 rounded-lg shadow border border-gray-200 dark:border-neutral-700">
            ...
            <div>
                <div class="flex gap-4 items-end">
                    ...
                    <div>
                        ...
                        <select id="bulkValue"
                            class="bg-white placeSelect dark:bg-neutral-800 border border-gray-300 dark:border-neutral-600 text-gray-800 dark:text-white rounded px-3 py-1 mt-1">
        ...
        <form id="purchaseForm">
            ...
            <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700 group-block">
                <table id="purchaseTable" class="min-w-[900px] w-full text-sm text-left text-gray-600 dark:text-gray-200">
                    <colgroup>
...
<style>
    ...
    .inputCell { background:white; border:1px solid #d1d5db; color:#111827; padding:4px 6px; font-size:12px; width:100%; min-width:0; height:30px; border-radius:4px; }
...
<script>
    ...
    function buildItemRow(item) {
        ...
        return `<tr>
            ...
            <td style="width:80px;"><input type="text" class="item-hsn" inputmode="numeric" maxlength="8" pattern="\\d{4}|\\d{6}|\\d{8}" title="HSN Code must contain exactly 4, 6, or 8 digits" value="${item.hsn || ''}" placeholder="HSN" style="text-align:center;"></td>
    ...
    function addNoItemRow(data = {}) {
        ...
        let row = `
            <tr>
                ...
                <td>
                    <select class="receipt-input noitem-gst" style="text-align:right;">${buildGstRateOptions(data.gst || 0)}</select>
                </td>
                <td>
    ...
    $('#updateRow').click(function () {
        if (!validateBulkInvoiceFrontend({ headerLedger: '#noitem_purchase_ledger', ledgerName: 'Purchase Ledger', itemSelector: '.item-name', gstSelector: '.item-gst_rate', qtySelector: '.item-qty', unitSelector: '.item-unit', rateSelector: '.item-rate', amountSelector: '.item-amount', hsnSelector: '.item-hsn' })) return;
        let items = [];
    ...
    function collectCustomSlots() {
        ...
        $('#customSlotsBody tr').each(function () {
            ...
            slots.push({
                ...
                purchase_ledger_id: row.data('purchase-ledger-id') || row.find('.slot_purchase_ledger_id').val() || null,
    ...
    function renderCustomSlots(rateMap, grandTotal) {
        ...
        allRates.forEach(function(rate) {
            ...
            let halfR  = parseFloat(displayRate) / 2;
            ...
            slotHtml += `<tr class="${isZero ? 'zero-row' : ''}" data-rate="${displayRate}" data-slot-key="${slotKey}" data-purchase-ledger-id="${data.ledgerId || ''}">
                <td><span class="rate-badge"><span class="slot-rate"></span>${displayRate}%</span></td>
                <td class="slot-taxable"><strong>${fmt(data.amt)}</strong><input type="hidden" class="slot_purchase_ledger_id" value="${data.ledgerId || ''}"></td>
                <td><select class="slot-igst-ledger" data-rate="${displayRate}"><option value="">— Ledger —</option>${iOpts}</select></td>
                <td><input type="number" class="slot-igst-amt" data-rate="${displayRate}" value="${igstAmt.toFixed(2)}" step="any"></td>
                <td><select class="slot-cgst-ledger" data-rate="${displayRate}"><option value="">— Ledger —</option>${cOpts}</select></td>
                <td><input type="number" class="slot-cgst-amt" data-rate="${displayRate}" value="${cgstAmt.toFixed(2)}" step="any"></td>
                <td><select class="slot-sgst-ledger" data-rate="${displayRate}"><option value="">— Ledger —</option>${sOpts}</select></td>
                <td><input type="number" class="slot-sgst-amt" data-rate="${displayRate}" value="${sgstAmt.toFixed(2)}" step="any"></td>
            </tr>`;
        });
    ...
    $('#noitem_gst_rate, #noitem_amount').on('input change', function () {
        recalcNoItemGST();
    });
