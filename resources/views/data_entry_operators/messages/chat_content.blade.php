...
<div
    ...
    {{-- Messages area --}}
    @php $chatInstance = $chat_instance ?? 'chat'; @endphp
    <div id="{{ $chatInstance }}ChatScroll" class="flex-1 min-h-0 flex flex-col p-3 overflow-y-auto bg-black">
        @php
            $prevDay = null;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const scroll = document.getElementById('{{ $chatInstance }}ChatScroll');
        ...
        function appendOutgoingMessage(text) {
            ...
            avatar.className = 'mr-2 mt-0.5 shrink-0';
            avatar.innerHTML = '<div class="w-7 h-7 rounded-full grid place-items-center text-[11px] font-semibold bg-indigo-600 text-white">D</div>';

            const bubble = document.createElement('div');
            bubble.className = 'max-w-[78%] md:max-w-[66%] px-3 py-2 rounded-2xl leading-snug bg-indigo-600 text-white rounded-br-none';

            const content = document.createElement('div');
            content.className = 'flex items-end justify-between gap-2';

            const body = document.createElement('p');
            body.className = 'whitespace-pre-line break-words flex-1 leading-snug';
            body.textContent = text;

            const time = document.createElement('span');
            time.className = 'text-[10px] opacity-80 shrink-0 ml-2 whitespace-nowrap self-end';
        ...
        // Send in the background so the conversation behaves like an instant messenger.
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                textarea.value = formData.get('body') || '';
                updateSendButton();
    ...
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            mutation.addedNodes.forEach(function(node) {
                if (node.nodeType === 1 && node.id === '{{ $chatInstance }}ChatScroll') {
