            ...
            <a href="{{ route('deo.messages.index', ['client' => $c->id]) }}"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="font-medium truncate {{ $isActive ? 'text-indigo-900 dark:text-indigo-100' : 'text-gray-900 dark:text-white' }}">
                            {{ $c->client_name ?? ($c->name ?? 'Unknown') }}
                        </div>
                        @if (!empty($c->last_message))
                            <div class="text-xs truncate {{ $isActive ? 'text-indigo-700 dark:text-indigo-300' : 'text-gray-500 dark:text-gray-400' }}">
                                {{ $c->last_message }}
                            </div>
                        @endif
                    </div>
