<div class="flex flex-col h-[calc(97vh-100px)] bg-white dark:bg-neutral-800 !rounded-full" data-controller='client--message'>
    <div class="flex items-center gap-4 px-6 py-4 border-b border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 sticky">
        <img src="{{ asset('images/light-logo.svg') }}" alt="light-logo" class="light-logo block dark:hidden">
        <img src="{{ asset('images/dark-logo.svg') }}" alt="dark-logo" class="dark-logo hidden dark:block">
    </div>

    <div class="flex-1 overflow-y-auto p-6 bg-white dark:bg-neutral-800 space-y-4 messageBox-{{ auth()->id() }}" id="messages" data-is-client="true">
        <div id="client_message_blocks">
            @if($messages->isNotEmpty())
                @php $last_message_date = null @endphp
                @foreach($messages as $message)
                    @php $current_message_date = $message->created_at->toDateString() @endphp
                    @if($last_message_date != $current_message_date)
                        <div class="flex justify-center my-4">
                            <span class="px-3 py-1 text-xs bg-neutral-200 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300 rounded-full">
                                @if($current_message_date == now()->toDateString())
                                    @lang('chat.messages.today')
                                @elseif($current_message_date == now()->subDay()->toDateString())
                                    @lang('chat.messages.yesterday')
                                @else
                                    {{ $message->created_at->formatLocalized('%B %d, %Y') }}
                                @endif
                            </span>
                        </div>
                        @php $last_message_date = $current_message_date @endphp
                    @endif

                    @include('client.messages.message_block', ['message' => $message])
                @endforeach
            @else
                <div class="flex flex-col items-center justify-center h-full text-center py-10 px-6">
                    ...
                    <h3 class="text-xl font-medium text-neutral-800 dark:text-white mb-2">
                        @lang('chat.messages.no_messages.title')
                    </h3>
                    <p class="text-neutral-600 dark:text-neutral-300 text-sm">
                        @lang('chat.messages.no_messages.description', ['name' => 'Balantro'])
                    </p>
                </div>
            @endif
        </div>
    ...
    <div class="border-t border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-4 sticky bottom-0 z-20" id="message_form">
        <form action="{{ route('client.messages.store') }}" 
              method="POST"
            ...
            <input type="hidden" name="is_first_message" value="{{ $messages->isEmpty() ? 'true' : 'false' }}">
            ...
            <div id="file-preview-container" class="hidden mb-2" data-message-target="filePreviewContainer">
                <div class="flex items-center gap-2 bg-neutral-100 dark:bg-neutral-700 rounded-lg px-3 py-2 overflow-x-scroll">
                    ...
                    <button type="button" 
                            ...
                            title="@lang('chat.messages.file_input.clear_all')">
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="button" 
                        ...
                        title="@lang('chat.messages.file_input.attach')">
                ...
                <div class="flex-1 relative">
                    <input type="text" name="description" 
                          ...
                          autocomplete="off" 
                          placeholder="@lang('chat.messages.input.placeholder')"
