<div class="flex {{ $sender ? 'justify-end' : 'justify-start' }}" id="message-{{ $message->id }}">
    <div class="flex max-w-[80%] gap-3">
        @if(!$sender)
            <div class="avatar-lg object-fit-cover rounded-full bg-primary-500 text-white flex items-center justify-center flex-shrink-0" style="width: 32px; height: 32px;">
                {{ $message->sender->is_client ? strtoupper(substr($message->sender->name, 0, 1)) : 'B' }}
            </div>
        @endif

        <div class="flex flex-col {{ $sender ? 'items-end' : 'items-start' }}">
            ...
            @if($message->description)
                <div class="px-4 py-3 rounded-2xl {{ $sender ? 'bg-primary-500 text-white rounded-br-none' : 'bg-neutral-100 dark:bg-neutral-700 text-neutral-800 dark:text-neutral-200 rounded-bl-none' }}">
