    ...
    <div class="px-2 py-3 sm:px-3">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mt-3">
            ...
            <div class="group relative p-3 rounded-xl text-center
                ...
                <p class="relative text-lg font-bold text-gray-900 dark:text-white">{{ $uploaded_count }}</p>
            </div>
            ...
            <div class="group relative p-3 rounded-xl text-center
                ...
                <p class="relative text-lg font-bold text-gray-900 dark:text-white">{{ $in_progress_count }}</p>
            </div>
            ...
            <div class="group relative p-3 rounded-xl text-center
                ...
                <p class="relative text-lg font-bold text-gray-900 dark:text-white">{{ $completed_count }}</p>
            </div>
            ...
            <div class="group relative p-3 rounded-xl text-center
                ...
                <p class="relative text-lg font-bold text-gray-900 dark:text-white">{{ $rejected_count }}</p>
            </div>
            ...
            <div class="group relative p-3 rounded-xl text-center
                ...
                <p class="relative text-lg font-bold text-gray-900 dark:text-white">{{ $accepted_count }}</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 w-full">
                ...
                <!-- <div class="w-full relative">
                    ...
                    <select name="client_id"
                        ...
                        <option value="{{ $c->id }}"
                            {{ (int) request('client_id') === (int) $c->id ? 'selected' : '' }}>
                            {{ $c->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="relative"
                        ...
                        <ul x-show="open" @click.outside="open = false"
                            ...
                            <li>
                                <button type="button"
                                    @click="selected = '{{ $c->id }}'; open = false"
                                    class="w-full text-left px-4 py-2 text-sm
                                        transition-all duration-200
                                        text-gray-800 dark:text-white
                                        hover:bg-black/10 dark:hover:bg-white/10
                ...
                <!-- <div class="relative">
                    ...
                    <select name="status"
                        ...
                        <option value="{{ $val }}" {{ request('status', 'all') === $val ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="relative"
                    x-data="{ open: false, selected: '{{ request('status', 'all') }}', options: {{ json_encode($statuses) }} }">
                ...
                <div class="w-full">
                    ...
                    <input type="date" name="start_date" value="{{ request('start_date') }}" min="1900-01-01"
       max="2099-12-31"
                ...
                <div class="w-full">
                    ...
                    <input type="date" name="end_date" value="{{ request('end_date') }}" min="1900-01-01"
       max="2099-12-31"
                        class="w-full appearance-none bg-gradient-to-br from-white/50 to-white/20 dark:from-white/10 dark:to-transparent backdrop-blur-xl border border-gray-300/80 dark:border-cyan-400/20 shadow-[inset_0_1px_2px_rgba(255,255,255,0.4)] dark:shadow-[inset_0_1px_2px_rgba(255,255,255,0.05)] text-gray-900 dark:text-white rounded-xl px-3 py-2 text-sm pr-10 focus:outline-none focus:ring-2 focus:ring-[#22d3ee] focus:border-[#22d3ee] focus:shadow-[0_0_12px_rgba(34,211,238,0.6)] transition-all duration-300">
                </div>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800 tabular-nums">
                ...
                <tr class="group transition-all duration-300 hover:bg-[#22d3ee]/80 dark:hover:bg-[#22d3ee]/80 hover:shadow-[0_0_20px_rgba(34,211,238,0.8)] [&>*]:group-hover:text-black [&_*]:group-hover:text-black">
                    ...
                    <td class="w-44 min-w-[11rem] px-4 py-2 text-center group-hover:text-black">
                        ...
                        @endphp
                        <span class="inline-flex min-h-7 min-w-40 items-center justify-center whitespace-nowrap rounded-full px-4 py-1 text-center text-xs font-semibold leading-tight {{ $statusClass }}">
                            {{ $statusLabel }}
                        </span>
                    </td>
                    <td class="px-4 py-2 group-hover:text-black">
                        {{ optional($doc->updated_at)->timezone(config('app.timezone'))->format('d M, Y, h:i A') }}
                    </td>
                    <td class="px-4 py-2 group-hover:text-black">
                        <div class="flex items-center justify-end gap-1 sm:gap-2">
                            ...
                            @if ($user->role === \App\Models\User::ROLES['data_entry_operator'])
                            <a href="{{ route('documents.financial-management', $doc->id) }}"
                            ...
                            {{-- Data Entry Operator Verify Button --}}
                            @if ($user->role === \App\Models\User::ROLES['data_entry_operator'])
                            @can('documents.verify')
                            @if (!in_array(trim($doc->status), ['approved', 'data_entry_completed']))
                            <button type="button"
                                onclick="openVerifyModal({{ $doc->id }},'{{ $doc->status }}')"
                            ...
                            @can('documents.view-activities')
                            <a href="{{ route('documents.activities', $doc) }}" title="Activity Log"
    ...
    <div id="verifyDocumentModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-lg dark:bg-neutral-900">
            ...
            <form method="POST" action="{{ route('documents.verify') }}" class="mt-2 space-y-4">
                @csrf
    ...
    <div id="supVerifyModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-lg dark:bg-neutral-900">
            ...
            <form method="POST" action="{{ route('documents.sup_verify') }}" class="mt-4 space-y-4">
                @csrf
